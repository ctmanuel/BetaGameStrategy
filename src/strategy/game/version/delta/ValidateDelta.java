 ...
 * @version Oct 03, 2013
 */
public class ValidateDelta implements Validate {

	InitializeDelta initializeDelta = new InitializeDelta();
	...
	public void checkPiecesDistributed(Map<PieceType, Integer> piecesUsed)
			throws StrategyException {
		for (PieceType pt : initializeDelta.getStartingPieces().keySet()) {
			int required = initializeDelta.getStartingPieces().get(pt).intValue();
